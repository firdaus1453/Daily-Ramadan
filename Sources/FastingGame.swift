...
public class FastingGame: UIView {
    ...
    var popUpContainer: UIView = {
        ...
        m_view.backgroundColor = Helper.backgroundFasting
        return m_view
    }()
    ...
    func randomImage() {
        let backgroundImages = ["study","work","exercise","eat", "drink"]
    ...
    func gameEnd(){
        ...
        descEnd.text = """
        Good job, sunset is coming, we are ready for iftar.

        Fasting in Ramadan is the ultimate exercise in restraint and self-control, not just for fasting.
        """
    ...
    func retryScreen() {
        self.removeFromSuperview()
        let fastingGame = FastingIntro(scene: self)
